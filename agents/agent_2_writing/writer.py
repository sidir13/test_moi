...
class ScenarioWriterAgent:
    ...
    def __init__(self, client):
        ...
        self.system_prompt = (
            ...
            "2. **Respectez STRICTEMENT les durées cibles** : un récit "
            "narratif audio se lit à environ **2 mots par seconde** "
            "(120 mots/min). Pour une cible de 120s → 240 mots MAX. "
            "Comptez vos mots et condensez si nécessaire.\n"
            "3. **Adaptez-vous** : ton, rythme, style narratif définis par la "
            "configuration — suivez-les fidèlement.\n"
            "4. **Après une citation** : ne résumez/paraphrasez JAMAIS ce qui "
            "vient d'être cité — avancez vers un nouvel élément narratif.\n\n"
            "INTERDICTION ABSOLUE — PHRASES MÉTA :\n"
            "N'écrivez JAMAIS de phrase qui décrit le récit lui-même, son ton, "
            "son public ou sa forme. Sont STRICTEMENT INTERDITES :\n"
            "- « Ce récit s'adresse à… »\n"
            "- « Le ton sera celui du… »\n"
            "- « Bienvenue dans ce documentaire »\n"
            "- « Dans ce programme… »\n"
            "- Toute phrase qui parle DE l'histoire plutôt que de LA raconter.\n"
            "Commencez IMMÉDIATEMENT dans l'histoire, avec une image, un fait, "
            "une scène — jamais une introduction méta.\n\n"
            "INTÉGRATION DES CITATIONS — RÈGLES PRÉCISES :\n"
            "Chaque citation [ARCHIVE : « … »] doit être INTRODUITE par un verbe "
            "d'attribution ancré dans le récit, sans jamais paraphraser après.\n"
            "Verbes d'attribution à utiliser : raconte, confie, se souvient, "
            "témoigne, précise, glisse, ajoute, murmure, explique — accordés à "
            ...
            "une IMAGE NARRATIVE — pas de résumé, pas d'écho du contenu cité.\n\n"
    ...
    def writeFromPromptTemplate(
        ...
        for part in structureParts:
            structureBlock += (
                ...
                f"(~{int(part.get('duree_cible', 0) * 2)} mots MAX)\n"
    ...
    def _write_all_parts_single_request(
        ...
        for ps in structure.get("structure", []):
            parts_desc.append(
                ...
                f"- Durée : {ps['duree_cible']}s (~{int(ps['duree_cible'] * 2)} mots MAX)\n"
