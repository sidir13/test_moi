...
def load_audio_selection(project_name: str) -> Dict[str, List[str]]:
    path = _project_dir(project_name) / "audio_selection.json"
    if not path.exists():
        return {"voices": [], "backgrounds": []}
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
...
def save_audio_selection(project_name: str, selection: Dict[str, List[str]]) -> Dict[str, List[str]]:
    payload = {
